...
zone_summary <- dt_zoned %>%
  group_by(Zone) %>%
  summarise(
    Avg_Price_Lakhs = median(Price_in_Lakhs, na.rm = TRUE),
    House_Count = n()
  ) %>%
  arrange(desc(Avg_Price_Lakhs))


bp = barplot(zone_summary$Avg_Price_Lakhs, 
        names.arg = zone_summary$Zone, 
        col = "darkorange",
        main = "Average House Price by Indian Zone",
        ylab = "Price (in Lakhs)",
        las = 1,
        ylim = c(0, max(zone_summary$Avg_Price_Lakhs) * 1.2))



text(x = bp, 
     y = zone_summary$Avg_Price_Lakhs, 
     labels = round(zone_summary$Avg_Price_Lakhs, 2), 
     pos = 3,         
     cex = 0.9,       
     font = 2,        
     col = "black")
