...
p1 <- plot_ly() %>%
  ...
  # Line for Overall Median
  add_trace(data = yearly_median, x = ~Year_Built, y = ~Price_in_Lakhs, name = 'Overall Mean',
            type = 'scatter', mode = 'lines+markers', line = list(color = 'red', dash = 'dash')) %>%
  
  # Layout and Axis Titles
  layout(title = "Yearly Median Price Trends: Ready_to_Move vs Under_Construction",
         xaxis = list(title = "Year Built"),
         yaxis = list(title = "Price (in Lakhs)", range = c(240, 270)),
         hovermode = "x unified") # Shows all values when you hover over a year
...
plot(yearly_median_ready,  type = "o", 
     ...
     xlab = "Year Built", 
     ylab = "Price (per SqFt)",
     main = "Yearly Median Price per SqFt Trends: Ready_to_Move vs Under_Construction")
